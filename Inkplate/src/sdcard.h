...
void saveToGalery(String url, bool show);
void saveToPath(String url, String path, bool show);
...
void playGallery()
{
    ...
    if (count)
    {
        ...
        sprintf(buffer, "%s/%d.jpeg", galeryFolderPath, random(0, count));
        Serial.printf("%s, max: %d\n", buffer, count);
        display.clearDisplay();
        display.drawJpegFromSd(buffer, 0, 0, 1, 0);
        display.display();
...
//         sprintf(buffer, "%s%d.jpeg", galeryFolderPath.c_str(), random(0, count));
...
void saveToPath(String url, String path, bool show)
{
    ...
    http.getStream().setTimeout(30000);
    http.begin(url);
    Serial.print(url);
    ...
    if (httpCode == 200)
    {
        ...
        uint8_t buff[256] = {0};
        SdFile file;
        file.open(path.c_str(), FILE_WRITE);
    ...
    {
        Serial.printf("\ndrawJpegFromSd: %s\n", path.c_str());
        display.drawJpegFromSd(path.c_str(), 0, 0, 1, 0);
    }
}
void saveToGalery(String url, bool show)
{
    char buffer[25];
    int count = countPhotos(galeryFolderPath.c_str());
    sprintf(buffer, "%s/%d.jpeg", galeryFolderPath.c_str(), count + 1);
    saveToPath(url, String(buffer), show);
    if (show)
    {
        display.drawJpegFromSd(buffer, 0, 0, 1, 0);
        display.display();
